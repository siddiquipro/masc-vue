import SCard from "./card/s-card.vue";
import SBtn from "./button/s-btn.vue";
import SAppbar from "./appbar/s-appbar.vue";
import { STable, SPagination, STableColumn, STableHead } from "./table";
import { SModal } from "./modal";
import { SIcon } from "./icon";

export default { SBtn, SCard, SAppbar, STable, SPagination, STableColumn, STableHead, SModal, SIcon };

export { SBtn, SCard, SAppbar, STable, SPagination, STableColumn, STableHead, SModal, SIcon };
